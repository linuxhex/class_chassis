...
Charger::Charger(){
    ...
    ros::NodeHandle charger_nh("~/device/charger");
...
void Charger::onCharge(void){
    if(sleep_cnt == 0){
        charger_cmd = CMD_CHARGER_ON;
        std::thread([&](){
          GS_INFO("[CHASSIS] start to check charger volatage = %d", charger_voltage);
          sleep(3);
          int check_charger_cnt = 0;
          while(++sleep_cnt  < 60 && charger_cmd == CMD_CHARGER_ON) {
           GS_INFO("[CHASSIS] checking charger volatage = %d", charger_voltage);
           if (charger_voltage >= low_voltage) {
             ++check_charger_cnt;
           } else {
             check_charger_cnt = 0;
           }
           if (check_charger_cnt > delay_time && charger_cmd == CMD_CHARGER_ON) {
...
void Charger::chargeValueManage(void)
{
    ...
    if (charger_cmd == CMD_CHARGER_MONITOR && charger_voltage < low_voltage && current_charge_voltage > low_voltage) {
      timeval tv;
      gettimeofday(&tv, NULL);
      charge_on_time = static_cast<double>(tv.tv_sec) + 0.000001 * tv.tv_usec;
     }
    charger_voltage = current_charge_voltage;
    GS_INFO("[wc_chassis] adc_charge = %d, charger_voltage_: %lf", charge_ADC, charger_voltage);

    if(p_battery != NULL && full_voltage < p_battery->battery_value){
       p_chassis_mcu->setChargeCmd(CMD_CHARGER_OFF);
       charger_cmd    = CMD_CHARGER_OFF;
       charger_status = STA_CHARGER_OFF;
       return;
    }

    if (charger_relay) {
      charger_status = STA_CHARGER_ON;
      double mileage = (p_chassis_mcu->mileage_right_ + p_chassis_mcu->mileage_left_) / 2;
      if(charger_voltage < low_voltage || (fabs(mileage - pre_mileage) >= 0.03)){
          p_chassis_mcu->setChargeCmd(CMD_CHARGER_OFF);
          charger_cmd    = CMD_CHARGER_OFF;
          charger_status = STA_CHARGER_OFF;
      }
    } else if (charger_voltage > low_voltage) {
      charger_status = STA_CHARGER_TOUCHED;
      onCharge();
    } else {
      charger_status = STA_CHARGER_OFF;
    }
    GS_INFO("[wc_chassis] get relay status = 0x%.2x, charger_status_ = %d", relay_status_, charger_status);
